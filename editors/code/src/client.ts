  ...
  const raMiddleware: lc.Middleware = {
    workspace: {
      ...
      async configuration(
        ...
        // The ritobin-lsp LSP only ever asks for the "ritobin-lsp"
    ...
    async handleDiagnostics(
      ...
      diagnosticList.forEach((diag, idx) => {
        ...
        if (
          ...
          const config = vscode.workspace.getConfiguration("ritobin-lsp");
        ...
        const rendered = (diag as unknown as { data?: { rendered?: string } })
          .data?.rendered;
        if (rendered) {
          if (preview) {
      ...
      const callback = async (
        ...
        for (const item of values) {
          ...
          const mkAction = () => {
            ...
            action.command = {
              command: "ritobin-lsp.resolveCodeAction",
              title: item.title,
              arguments: [item],
            };
        ...
        for (const [group, { items, primary }] of groups) {
          ...
          if (items.length !== 0) {
            ...
            primary.title = group;
            primary.command = {
              command: "ritobin-lsp.applyActionGroup",
              title: "",
              arguments: [args],
            };
          }
...
class ExperimentalFeatures implements lc.StaticFeature {
  ...
  fillClientCapabilities(capabilities: lc.ClientCapabilities): void {
    capabilities.experimental = {
      ...
      commands: {
        commands: [
          "ritobin-lsp.runSingle",
          "ritobin-lsp.debugSingle",
          "ritobin-lsp.showReferences",
          "ritobin-lsp.gotoLocation",
          "ritobin-lsp.triggerParameterHints",
          "ritobin-lsp.rename",
        ],
      },
      ...capabilities.experimental,
...
function renderCommand(cmd: ra.CommandLink): string {
  ...
  return `[${cmd.title}](command:ritobin-lsp.hoverRefCommandProxy?${
    HOVER_REFERENCE_COMMAND.length - 1
  } '${cmd.tooltip}')`;
}

function renderHoverActions(
  actions: ra.CommandLinkGroup[],
